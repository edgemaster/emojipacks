...
function Slack(opts, debug) {
  ...
  this.import = function *() {
    ...
    for (var i = 0; i < Object.keys(this.opts.emojis).length; i++) {
      ...
      if (e.src) {
        var deleteRes = yield this.del(e.name);
  ...
  this.emoji = function *() {
    ...
    opts.deleteCrumb = $('.emoji_row input[name="crumb"]').first().attr('value');
    console.log("Upload crumb is %s", opts.uploadCrumb);
    console.log("Delete crumb is %s", opts.deleteCrumb);
    if (!opts.uploadCrumb) throw new Error('Login error: could not get emoji upload crumb for ' + opts.url);
    if (!opts.deleteCrumb) throw new Error('Login error: could not get emoji delete crumb for ' + opts.url);
    return this.opts = opts;
  };
  ...
  this.del = function *(name) {
    ...
    return new Promise(function(resolve, reject, notify) {
      ...
      var form = r.form();
      form.append('crumb', opts.deleteCrumb);
